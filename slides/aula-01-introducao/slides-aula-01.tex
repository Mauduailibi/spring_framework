...
\begin{frame}{Passo 1 --- Instalar o JDK 21}
  \begin{enumerate}
    ...
    \item Aceite as opções padrão (marque a opção \textbf{Add to PATH} se aparecer)
  \end{enumerate}
\end{frame}
...
% #############################################################################
\section{PathVariable (nível desafio)}

% =============================================================================
\begin{frame}{Outro jeito de passar valor na URL}
  Vocês já usaram \texttt{?nome=Ana} (\texttt{@RequestParam}).

  \vspace{0.5em}
  Agora o valor vai \textbf{no caminho}:

  \begin{center}
  \begin{tabular}{ll}
    \toprule
    Query & \texttt{/saudacao?nome=Ana} \\
    Caminho & \texttt{/saudacao/Ana} \\
    \bottomrule
  \end{tabular}
  \end{center}

  \vspace{0.6em}
  A anotação é \texttt{@PathVariable}.\\
  Dá para usar \textbf{um ou vários} pedaços no caminho.
\end{frame}

% =============================================================================
\begin{frame}[fragile]{Sintaxe rápida}
\begin{lstlisting}[language=Java,basicstyle=\ttfamily\footnotesize]
...
\end{lstlisting}
  ...
  \vspace{0.3em}
  Teste: \texttt{http://localhost:8080/saudacao/Maria}
\end{frame}

% =============================================================================
\begin{frame}{Desafios PathVariable (na sala)}
  Três exercícios um pouco mais pesados --- ainda no \texttt{ola-spring}.

  \vspace{0.4em}
  \begin{enumerate}
    \item \textbf{\texttt{/calc/\{a\}/\{b\}/\{op\}}}\\
          Ex.: \texttt{/calc/10/2/div} $\rightarrow$ \texttt{10 / 2 = 5}\\
          \texttt{op}: \texttt{soma}, \texttt{sub}, \texttt{mult}, \texttt{div}.\\
          Divisão por zero: mensagem amigável.
    \item \textbf{\texttt{/usuario/\{id\}/pedidos/\{pedidoId\}}}\\
          Ex.: \texttt{/usuario/7/pedidos/42}\\
          $\rightarrow$ \texttt{Usuario 7, pedido 42 encontrado.}
    \item \textbf{\texttt{/frete/\{peso\}/\{distancia\}} + query}\\
          Ex.: \texttt{/frete/5/100?urgente=true}\\
          Regra base: \texttt{preco = peso * distancia * 0.5}\\
          Se \texttt{urgente=true}, acrescente 20\%.\\
          Mostre o preço final.
  \end{enumerate}
\end{frame}

% =============================================================================
\begin{frame}{Dicas para os 3 desafios}
  \begin{itemize}
    \item Vários \texttt{\{...\}} = vários \texttt{@PathVariable}
    \item No exercício 3, misture:\\
          \texttt{@PathVariable} + \texttt{@RequestParam boolean urgente}
    \item Se \texttt{urgente} puder faltar, use\\
          \texttt{@RequestParam(defaultValue = "false")}
    \item Reinicie o Spring depois de cada tentativa que salvar
  \end{itemize}

  \vspace{0.6em}
  \begin{alertblock}{Meta}
    Provar que vocês leem a URL, escolhem path ou query,\\
    e resolvem a lógica em Java --- como no dia a dia de API.
  \end{alertblock}
\end{frame}
...
\begin{frame}{O que aprendemos hoje}
  \begin{enumerate}
    ...
    \item \texttt{@RequestParam} lê valores depois do \texttt{?}
    \item \texttt{@PathVariable} lê valores no caminho da URL
